 ...
 */
// 看注释应该是一个count只有0或者1的file lookup流程
...
static unsigned long __fget_light(unsigned int fd, fmode_t mask)
{
	...
	if (atomic_read(&files->count) == 1) {
		file = __fcheck_files(files, fd); // 即使是下面的count==0的情况，最终也是调用到__fcheck_files
		...
		if (!file)
			return 0;
		// FDPUT_FPUT使得fdput()/fput()调用有效，代表read结束后需要减小文件引用计数
...
{
	return (struct fd){(struct file *)(v & ~3),v & 3}; // long分为file*和int flag
}