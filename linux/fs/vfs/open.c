...
// 整个过程十分不建议看，感觉非常无聊
//
// 简单描述一下不带O_CREAT的open流程（简单理解，可能不完全吻合）
// 1. 构造open_flags，这些都是影响open流程的状态机参数
// 2. 从用户空间拷贝filename到内核空间，其内核实例就是struct filename
// 3. 分配fd
// 4. 初始化表示路径查找上下文的nameidata，这是存储多个分量查找/解析过程的临时类型
// 5. **启用rcu-walk**，先说疗效，就是得到打开文件struct file
// 6. fsnotify
// 7. fd和file关联
//
// 第5点是关键操作
// 由于要处理众多问题，比如open时的mode和flag，比如符号链接，.和..，目录根本就没有，mount路径等等麻烦问题，
// 而且还要用上RCU操作，因此变得非常复杂
// 我展开说一下最为简略的过程
// 5.1. 获得空分配的file实例
