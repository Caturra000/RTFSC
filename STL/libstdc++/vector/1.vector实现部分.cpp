// vector直接protected复用vector_base，其关键成员在vector_base里面（allocate而不initialize）

// 初始化策略 _M_default_initialize -> std::__uninitialized_default_n_a
//     可以理解为对n个对象直接allocator::construct
//     construct接口为(alloc, addr, args...)，会针对addr地址进行operator new
//     而__uninitialized_default_n_a这里args为空
//     也就是要求typename _Tp必须要有空构造函数才行？

// Question. _S_relocate等函数是用于？
